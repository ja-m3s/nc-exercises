...
app.get("/random-trivia", async (req, res) => {
  const randomIndex = Math.floor(Math.random() * trivia.length);
  const trivium = randomIndex === 0 ? trivia[0] : trivia[randomIndex];
  counter.inc();
  console.log(await client.register.metrics());
  res.status(200).send({ trivium });
});

// Endpoint to get a random cool fact
app.get("/cool-fact", async (req, res) => {
  const randomIndex = Math.floor(Math.random() * coolFacts.length);
  const fact = randomIndex === 0 ? coolFacts[0] : coolFacts[randomIndex];
  counter.inc();
  console.log(await client.register.metrics());
